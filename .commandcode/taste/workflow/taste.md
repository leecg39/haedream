...
- QA should include E2E flows and may be requested repeatedly ("QA 완벽하게 해줘 (e2e 까지 포함) 5번 반복"), often together with debugging. "완벽하게" means exhaustive, not smoke-test: sweep every page/flow, verify writes at the DB/API level (not just what the UI shows), collect evidence (screenshots, console, captured network payloads), and report root-caused, severity-classified issues. Confidence: 0.75
...
g. Confidence: 0.7
- When a clean tree is needed, explicitly chose "모두 커밋" over the recommended partial commit: is comfortable committing all enumerated untracked items — including agent state dirs (.Codex/, .commandcode/, .omo/) and audit evidence/handoff docs — rather than stashing or excluding. Still stage enumerated paths explicitly, never blind `git add -A`. Confidence: 0.6
