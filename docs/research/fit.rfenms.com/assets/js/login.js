...
window.document.querySelector('title').textContent = window.location.hostname === 'fit.eggbz.com' ? '에그핏' : '한국미래에너지';
...
vio.setLogo = function() {
    ...
    if (vio.isEgg()) {
        ...
        loginLogo.src = './assets/img/eggfit_h.png';
    } else {
...
vio.login = async function() {
    ...
    let referrer = document.referrer || 'peak.html';
    ...
    if (!id) {
        ...
        const res = await fetch(`https://watt.rfenms.com/api/tokens`, {
        ...
        if (!res.ok) {
            ...
            if (token) {
                ...
                if(permit == '2'){
                    referrer = document.referrer || 'lowPeak.html';
                }
