...
vio.peakLoad = async function() {
    const res = await fetch(`${this._apiUrl}/api/peak-panels/${this._fid}`, {
        method: 'GET',
        headers: {
...
vio.controlRequest = async function(j) {
    ...
    if (ta.rTime + 30 < nowTime) {
        ...
        const res = await fetch(`${this._apiUrl}/api/controls/${this._fid}`, {
            method: 'POST',
            headers: {
                'Authorization': `x-auth ${vio._accessToken}`,
                'Content-Type': 'application/json;charset=utf-8'
            },
            body: `{"cf":"control","cid":${ta.cid},"authName":"${sessionStorage.getItem('authName')}"}`
        });
