...
2026-09-07 실업체 연동 후 추가 증거: 사용자가 제공한 업체 마스터 1,654건을 ADMIN 전용 DB에 연결했고, migration 013 및 암호화 자격증명을 포함한 반영 후 약 1.15GB 로컬 백업의 재개방·권한·복호화를 검증했다. [실업체 연동 검토](../audit/2026-09-07-private-firm-import-review.md) 참고. 이 CSV는 계측 시계열이 아니므로 A/P5의 승인 원본을 대체하지 않으며 외부 운영 DB·알림 수신·7일 관찰 상태도 바꾸지 않는다.
...
| 운영자 attestation | `ops:external-input` action `attest-large-db` (`iApproveAttestation=true`, operator, statement, expectedSha256). **체크박스 자동 변경 없음** |
...
| Day0 등록 | `register-observation` (plannedDays≥7). 등록 ≠ 7일 완료 |
...
| 환경 목록 attestation | `declare-external-env` (`iConfirmInventory=true`). 호스팅 env마다 offline snapshot + `audit-migration-011` 필요. inventory만으로 E 완료 금지 |
