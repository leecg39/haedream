...
vio._geocoder = new kakao.maps.services.Geocoder();
...
vio.deskEditFixed = async function() {
    ...
    if (!params.firmName) {
        ...
        const res = await fetch(`api/firm/${this._fid}`, {
            method: method,
            headers: {
...
vio.deskItem = async function(j) {
    if (!this._useNetworks) {
        ...
        const res = await fetch(`api/firm/${this._fid}/${this._sheet.idn}`, {
            method: 'GET',
            headers: {
                'Authorization': `x-auth ${vio._accessToken}`,
                'Content-Type': 'application/json;charset=utf-8'
...
vio.deskDropFixed = async function() {
    if (!this._useNetworks) {
        ...
        const res = await fetch(`api/firm/${this._fid}/${this._sheet.dropItem.getAttribute('data-idn')}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `x-auth ${vio._accessToken}`,
                'Content-Type': 'application/json;charset=utf-8'
...
vio.getData = async function(j) {
    if (!this._useNetworks) {
        ...
        const res = await fetch(`api/firm/${this._fid}?${queryString}`, {
            method: 'GET',
            headers: {
                'Authorization': `x-auth ${vio._accessToken}`,
                'Content-Type': 'application/json;charset=utf-8'
