...
vio.chart1 = function (legend, data) {
    ...
    function createSeries(name, valueY) {
        ...
        series.tooltipText = '[#fff]{name} :[/] [#fff bold]{valueY.value}[/][#e0e0e0]h[/]';
        series.name = name;
...
vio.dataTrans = function (device, data) {
    ...
    for (const sortItem of sortList) {
        ...
        // 부하율 정보
        out = `${out}<div class="chartItem">
            <span class="chartItemLabel">${dName}</span>
            <span>${(sortItem.un / 96 / timeLen * 100).toFixed(2)}%</span>
        </div>`;
    ...
    for (let uTime in timeData) {
        ...
        for (let pid in top10) {
            for (const deviceData of timeData[uTime]) {
                if (deviceData[0] == pid) {
                    ta[pid] = Math.floor(deviceData[1] / 96 * 24);
                }
            }
        }
        chartData.push(ta);
...
vio.getData = async function (isInit) {
    if (!this._useNetworks) {
        ...
        const res = await fetch(`api/tech-usages/${this._fid}?${queryString}`, {
            method: 'GET',
            headers: {
        ...
        if (!res.ok) {
            ...
            if (jsonData.code) {
                ...
                this.dataTrans(jsonData.device, jsonData.data);
                if(isInit){
